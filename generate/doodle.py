...
from downloadDatasets import download

def generate():
    classesPath      = 'doodle.txt'
    testImage        = 'images/doodle.png' 
    savedModelPath   = 'models/doodle.h5'
    savedClassesPath = 'models/doodle.txt'

    def load_data(root, vfold_ratio=0.2, max_items_per_class= 4000 ):

        all_files = glob.glob(os.path.join(root, '*.npy'))
        if len(all_files) == 0:
            print('\nNo data found in the data/doodle folder. Running downloadDatasets.py')
            success = download(classesPath)
            if not success:
                exit()
